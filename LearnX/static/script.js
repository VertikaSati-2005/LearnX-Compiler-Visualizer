/**
 * Legacy static UI helpers (primary app is React at /compiler).
 * Tab switching for any static prototype: toggle .active on tab + panel pairs.
 */
...
function renderOutput(data) {
  ...
  const oc = data && data.success ? prettyComplexity(data.overall_complexity || "—") : "—";
  overall.textContent = oc;
  overall.className = `badge ${badgeClass(overall.textContent)}`;
  if (data && data.success && data.overall_t_n != null) {
    overall.title = `T(n) = ${data.overall_t_n}`;
  } else {
    overall.title = "";
  }
  ...
  for (const item of data.lines) {
    ...
    if (item.t_n != null) {
      row.appendChild(el("span", "term-tn", `  T(n)=${item.t_n}`));
    }
